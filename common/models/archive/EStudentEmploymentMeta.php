...
/**
 *
 * Class EStudentEmploymentMeta
 * @package common\models\student
 */
class EStudentEmploymentMeta extends EStudentMeta
{
     ...
     */
    public function searchForEmployment($params, $department = null, $asProvider = true)
    {
        $this->load($params);
        ...
        $query->joinWith(
            ['student', 'level', 'department', 'group', 'specialty', 'educationYear', 'educationType', 'educationForm', 'paymentForm']
        );
    ...
    private function getSelectQueryFilters($col)
    {
        $query = self::find()->select([$col])
            ->andFilterWhere(['active' => true, '_student_status' => StudentStatus::STUDENT_TYPE_GRADUATED])
            ->distinct();

        foreach (['_department', '_education_form', '_education_year','_education_type', '_group', '_payment_form'] as $attribute) {
    ...
    public function getPaymentFormItems()
    {
        return ArrayHelper::map(
            PaymentForm::find()
                ->orderByTranslationField('name')
                ->where(['active' => true, 'code' => $this->getSelectQueryFilters('_payment_form')])
                ->all(), 'code', 'name');
    }
