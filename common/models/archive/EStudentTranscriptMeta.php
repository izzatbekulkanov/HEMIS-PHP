 ...
 */
class EStudentTranscriptMeta extends EStudentMeta
{
     ...
     */
    public function searchForTranscript($params, $department = null, $asProvider = true)
    {
        $this->load($params);
    ...
    private function getSelectQueryFilters($col)
    {
        $query = self::find()->select([$col])
            ->andFilterWhere(['active' => true, '_student_status' => StudentStatus::STUDENT_TYPE_STUDIED])
            ->distinct();
