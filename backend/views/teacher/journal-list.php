...
//                        'data' => ArrayHelper::map($dataProvider->getModels(), '_semester', 'semester.name'),
                    'allowClear' => true,
                    'hideSearch' => false,
                ])->label(false); ?>
            </div>
            <div class="col col-md-3">
                <?= $form->field($searchModel, '_group')->widget(Select2Default::classname(), [
                   ...
                   // 'data' => ArrayHelper::map($dataProvider->getModels(), '_group', 'group.name'),
                    'allowClear' => true,
                    'hideSearch' => false,
                ])->label(false); ?>
            </div>
            <div class="col col-md-3">
                <?= $form->field($searchModel, '_subject')->widget(Select2Default::classname(), [
                    'data' =>$searchModel->_group && $searchModel->_semester ? $searchModel->getSubjectItems() : [],
                    //'data' => ArrayHelper::map($dataProvider->getModels(), '_subject', 'subject.name'),
                    'allowClear' => true,
                    'hideSearch' => false,
                    'disabled' => !$searchModel->_group || !$searchModel->_semester,
                ])->label(false); ?>
            </div>
            <?php ActiveForm::end(); ?>
        </div>
    ...
    <?= GridView::widget([
        ...
        'columns' => [
			...
			[
                ...
                'format' => 'raw',
				'value' => function($data){
                        return Html::a($data->group->name,['attendance-journal', 'education_year' => $data->_education_year, 'semester' => $data->_semester, 'group' => $data->_group, 'subject' => $data->_subject, 'training_type' => $data->_training_type], ['data-pjax' => 0]);
                    },
			],
            [
                'attribute'=>'_subject',
                'format' => 'raw',
                'value' => function($data){
                    return Html::a($data->subject->name,['attendance-journal', 'education_year' => $data->_education_year, 'semester' => $data->_semester, 'group' => $data->_group, 'subject' => $data->_subject, 'training_type' => $data->_training_type], ['data-pjax' => 0]);
                },
            ],
            [
            ...
            [
                'attribute'=>'_semester',
                'value' => function ($data) {
                    return Semester::getByCurriculumSemester($data->group->_curriculum, $data->_semester)->name;
                },
            ],
           /* [
                'attribute'=>'_employee',
