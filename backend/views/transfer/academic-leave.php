...
use common\models\system\classifier\AcademicReason;
...
<div class="row">
                ...
                'columns' => [
                    [
                        ...
                        'checkboxOptions' => function (EStudentMeta $model) {
                            return [
                                'disabled' => !$model->canOperateAcademicLeave()
                            ];
                        }
                    ],
                    [
    ...
    <div class="col col-md-4" id="sidebar">
        <div class="box box-default ">
            <div class="box-body">
                <?php $form = ActiveForm::begin(['action' => ['/transfer/to-operation'], 'enableAjaxValidation' => false, 'validateOnSubmit' => false, 'options' => ['data-pjax' => 1, 'method' => 'post']]); ?>
                <?= $form->field($searchModelFix, '_status_change_reason')->widget(Select2Default::classname(), [
                    'data' => AcademicReason::getClassifierOptions(),
                    'allowClear' => false,
                    'hideSearch' => false,
                    'placeholder' => false,
                    'options' => [
                        'id' => '_status_change_reason'
                    ],
                ]); ?>
                <?php
                $decrees = EDecree::getOptionsByCurriculum($this->_user(), $searchModel->_curriculum ? $searchModel->curriculum : null, DecreeType::TYPE_ACADEMIC_LEAVE);
                ?>
...
<script>
    ...
    var operand = '<?= StudentStatus::STUDENT_TYPE_ACADEMIC; ?>';
    ...
    function assignStudents() {
        ...
        var order_reason = $('#_status_change_reason').val();

        if (decree == "" || decree == undefined) {
            alert(<?=json_encode([__('Buyruqni tanlang')])?>[0]);
        } else if (keys.length === 0) {
            alert(<?=json_encode([__('Talabalarni tanlang')])?>[0]);
        } else if (order_reason == "") {
            alert(<?=json_encode([__('Akademik ta\'til sababini tanlang')])?>[0]);
