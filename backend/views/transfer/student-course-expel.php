    ...
    <div class="col col-md-4" id="sidebar">
        <div class="box box-default ">
            ...
            <div class="box-footer text-right">
                <?= Html::a('<i class="fa fa-close"></i> ' . __('Clear Filter'), ['student-course-expel', 'clear-filter' => 1], [
            ...
            <div class="box-body">
                ...
                <?php
                $decrees = EDecree::getOptionsByCurriculum($this->_user(), $searchModel->_curriculum ? $searchModel->curriculum : null, DecreeType::TYPE_EXPEL_LEVEL);
                ?>
                <?= $form->field($searchModel, '_decree')->widget(Select2Default::class, [
                    ...
                    'options' => [
                        'id' => '_decree',
                        'required' => true,
                        'onchange' => 'return decreeChanged(this.value)'
                    ],
                    'disabled' => $searchModel->_semestr == null,
                    'hideSearch' => false,
                    'allowClear' => false,
                ]); ?>
                <?= $form->field($searchModel, 'order_date')->textInput(['id' => 'order_date', 'disabled' => true]) ?>
            </div>

            <div class="box-footer text-right">
                <?= Html::button('<i class="fa fa-check"></i> ' . __('Expel from Course'), [
...
<script>
    ...
    function confirmTransfer() {
        var keys = $('#data-grid').yiiGridView('getSelectedRows');
        if (keys.length > 0) {
            if (confirm(<?=json_encode([__('Are you sure to expel {count} students?')])?>[0].replace('{count}', keys.length))) {
...
<?php
$this->registerJs("decreeChanged($('#_decree').val());$('#data-grid input[type=\"checkbox\"]').on('change',function(){updateSelectedStudents()})")
?>
<?php Pjax::end() ?>
