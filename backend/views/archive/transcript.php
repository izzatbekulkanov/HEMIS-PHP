    ...
    <?= GridView::widget([
        ...
        'columns' => [
            ...
            [
                'attribute' => '_student',
                'format' => 'raw',
                'value' => function (EAcademicInformation $data) {
                    return Html::a(sprintf('%s<p class="text-muted">%s</p>', $data->student->getFullName(), $data->student->student_id_number), linkTo(['transcript-edit', 'transcript' => $data->id], ['data-pjax' => 0]));
                },
            ],
            ...
            [
                ...
                'format' => 'raw',
                'value' => function (EAcademicInformation $data) {
                    return sprintf('%s<p class="text-muted">%s</p>', $data->academic_register_number, Yii::$app->formatter->asDate($data->academic_register_date->getTimestamp()));
                },
            ],
            [
                'format' => 'raw',
                'value' => function (EAcademicInformation $data) {
                    return Html::a(__('View'), linkTo(['transcript-edit', 'transcript' => $data->id]), ['class' => 'btn btn-default btn-block', 'data-pjax' => 0]);
                },
            ],
        ],
    ]); ?>
