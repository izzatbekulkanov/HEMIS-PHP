...
class StudentCurriculum extends ECurriculum
{
    ...
    public function searchForStudent(Student $student, Semester $semester)
    {
        ...
        $query = ECurriculumSubject::find()
            ->where(['_curriculum' => $student->meta->_curriculum, 'active' => true])
            ->orderBy(['_semester' => SORT_ASC, 'position' => SORT_ASC])
            ->with(['subject', 'curriculumSubjectBlock', 'subjectType', 'semester', 'ratingGrade', 'subjectDetails']);
